...
def enderezar(img,perimetros_cromos_norm):
    filas,columnas=img.shape
    if filas>columnas:
    ...
    dst1=cortarImagen(S,dst,dst_gray,img,perimetros_cromos_norm[j][i])
    return dst1
...
if __name__ == "__main__":
        content_folder="single_clase"
        name_example_folder="97002138.9.tiff"
        ...
        areas_cromos_norm=areaPerim.normalizar_vector(areas_cromos)
        ...
        for j in range(0,len(string_path_img)):#j->clase
            for i in range(0,len(string_path_img[j])):#i ->img dentro de la clase
                img=areaPerim.load_image(string_path_img[j][i])
                tiempo_inicial = time()
                dst1=enderezar(img,perimetros_cromos_norm)
                tiempo_final = time()
                tiempo_ejecucion = tiempo_final - tiempo_inicial
                print('El tiempo de ejecucion fue:',tiempo_ejecucion)
                plt.subplot(121),plt.imshow(img,'gray'),plt.title('ORIGINAL')
                plt.subplot(122),plt.imshow(dst1,'gray'),plt.title('CORTADA')
                plt.show()
